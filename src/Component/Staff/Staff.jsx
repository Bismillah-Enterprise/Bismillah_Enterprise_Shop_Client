...
const Staff = () => {
    ...
    useEffect(() => {
        if (!wifiIp) return;
        fetch('https://shop-manager-server.onrender.com/set_ip')
        .then(res => res.json())
        .then(data => {
    ...
    //         fetch(`https://shop-manager-server.onrender.com/reset_time/${_id}`, {
    //             method: 'PUT',
    //             headers: {
                ...
                !message ?
                    <div>
                        ...
                        <div className='flex items-center gap-10 justify-center mt-10'>
                            ...
                            <button disabled={today_exit1_time === '' && isAllowed ? false : true} onClick={() => handleTodayTime('today_exit1_time', _id)} className='disabled:cursor-not-allowed disabled:bg-gray-400 disabled:opacity-60 rounded-full h-[100px] w-[100px] shadow-md shadow-pink-200 border-none text-pink-200 text-lg cursor-pointer hover:shadow-lg'>Exit 1</button>
                            <button disabled={today_enter2_time === '' && isAllowed ? false : true} onClick={() => handleTodayTime('today_enter2_time', _id)} className='disabled:cursor-not-allowed disabled:bg-gray-400 disabled:opacity-60 rounded-full h-[100px] w-[100px] shadow-md shadow-pink-200 border-none text-pink-200 text-lg cursor-pointer hover:shadow-lg'>Enter 2</button>
                            <button disabled={today_exit2_time === '' && isAllowed ? false : true} onClick={() => handleTodayTime('today_exit2_time', _id)} className='disabled:cursor-not-allowed disabled:bg-gray-400 disabled:opacity-60 rounded-full h-[100px] w-[100px] shadow-md shadow-pink-200 border-none text-pink-200 text-lg cursor-pointer hover:shadow-lg'>Exit 2</button>
                        </div>
